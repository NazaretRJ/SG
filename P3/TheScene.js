...
class TheScene extends THREE.Scene {
  ...
  createModel () {
    ...
    var textura = loader.load ("imgs/space.jpg");
    this.ground = new Ground (1000, 500, new THREE.MeshPhongMaterial ({map: textura}), 4);
    model.add (this.ground);
    this.flag = new Flag(this.ground.width,this.ground.deep);
    model.add(this.flag);
    ...
    this.MAX_DIS = this.ground.deep/5;
    model.add(this.zone);
    return model;
  }
//TODO ROBOT FUERA DE BORDES
  // Public methods
...
//if the robot camera is active,render can modify the global camera 
  ...
  IsWinner(){
    var win = false;
    var long = Math.abs(this.player.position.x - this.zone.position.x);
    var longz = Math.abs(this.player.position.z - this.zone.position.z);
    //var longy = Math.abs(this.player.position.y - objs.position.y);


    if(long <= this.MAX_DIS && longz <= this.MAX_DIS && this.player.getHaveFlag() == true){
      win = true;
  ...
  restart(){
    ...
    for(i = this.objs.length-1;i>0;i--)
      this.objs.remove(this.objs.child[i]);
    //regenarate all the objects
      for( i = 0; i < this.MAX_OBJS; i++){
        var objs_i = new Obstaculo(this.ground.width,this.ground.deep,
          this.flag.position.x,this.flag.position.z,
          this.player.position.x,this.player.position.z);
